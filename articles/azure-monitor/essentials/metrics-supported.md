...
ms.openlocfilehash: 77b1a826013f6093c7be8a2fca98b0df8c3bb7d3
ms.sourcegitcommit: b2daa3a26319be676c8e563a62c66e1d5e698558
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 03/05/2021
ms.locfileid: "102205292"
---
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|active-timer-count|是|System.Runtime|active-timer-count|计数|平均值|当前处于活动状态的计时器数|Deployment, AppName, Pod|
|alloc-rate|是|System.Runtime|alloc-rate|字节|平均值|托管堆中分配的字节数|Deployment, AppName, Pod|
|AppCpuUsage|是|应用 CPU 使用情况（预览版）|百分比|平均值|应用的最近 CPU 使用率|Deployment, AppName, Pod|
|assembly-count|是|System.Runtime|assembly-count|计数|平均值|已加载的程序集数|Deployment, AppName, Pod|
...
|exception-count|是|System.Runtime|exception-count|计数|总计|异常数|Deployment, AppName, Pod|
|failed-requests|是|Microsoft.AspNetCore.Hosting|failed-requests|计数|平均值|进程生存期内失败请求的总数|Deployment, AppName, Pod|
|gc-heap-size|是|System.Runtime|gc-heap-size|计数|平均值|GC 报告的堆总大小 (MB)|Deployment, AppName, Pod|
|gen-0-gc-count|是|System.Runtime|gen-0-gc-count|计数|平均值|第 0 代 GC 数|Deployment, AppName, Pod|
|gen-0-size|是|System.Runtime|gen-0-size|字节|平均值|第 0 代堆大小|Deployment, AppName, Pod|
|gen-1-gc-count|是|System.Runtime|gen-1-gc-count|计数|平均值|System.Runtime|第 1 代 GC 数|Deployment, AppName, Pod|
|gen-1-size|是|System.Runtime|gen-1-size|字节|平均值|第 1 代堆大小|Deployment, AppName, Pod|
|gen-2-gc-count|是|System.Runtime|gen-2-gc-count|计数|平均值|第 2 代 GC 数|Deployment, AppName, Pod|
|gen-2-size|是|System.Runtime|gen-2-size|字节|平均值|第 2 代堆大小|Deployment, AppName, Pod|
...
|monitor-lock-contention-count|是|System.Runtime|monitor-lock-contention-count|计数|平均值|尝试锁定监视器时出现争用的次数|Deployment, AppName, Pod|
|process.cpu.usage|是|process.cpu.usage|百分比|平均值|JVM 进程的最近 CPU 使用率|Deployment, AppName, Pod|
|requests-per-second|是|Microsoft.AspNetCore.Hosting|requests-rate|计数|平均值|请求速率|Deployment, AppName, Pod|
|system.cpu.usage|是|system.cpu.usage|百分比|平均值|整个系统的最近 CPU 使用率|Deployment, AppName, Pod|
|threadpool-completed-items-count|是|System.Runtime|threadpool-completed-items-count|计数|平均值|线程池已完成工作项计数|Deployment, AppName, Pod|
|threadpool-queue-length|是|System.Runtime|threadpool-queue-length|计数|平均值|线程池工作项队列长度|Deployment, AppName, Pod|
|threadpool-thread-count|是|System.Runtime|threadpool-thread-count|计数|平均值|线程池线程数|Deployment, AppName, Pod|
|time-in-gc|是|System.Runtime|time-in-gc|百分比|平均值|自上次 GC 以来 GC 的时间百分比|Deployment, AppName, Pod|
...
|working-set|是|System.Runtime|working-set|计数|平均值|进程所使用的工作集大小 (MB)|Deployment, AppName, Pod|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|RequestLatency|是|请求延迟|毫秒|总计|服务器处理请求所用的时间|Operation, Authentication, Protocol|
|RequestsTraffic|是|请求流量|百分比|计数|已发出的请求数|Operation, Authentication, Protocol, StatusCode, StatusCodeClass|
...
|OriginHealthPercentage|是|源运行状况百分比|百分比|平均值|从 AFDX 到后端的成功运行状况探测的百分比。|Origin, OriginPool|
...
|IngressEventsLatency|是|入口事件延迟|毫秒|平均值|从事件到达到准备好由 Azure 数字孪生传出的时间，此时服务将发送成功/失败结果。|结果|
...
|KafkaRestProxy.ProducerRequestTime.p95|是|REST 代理制造者 RequestLatency|毫秒|平均值|通过 Kafka REST 代理的制造者请求中的消息延迟|Machine, Topic|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|ApiLatency|否|ApiLatency|6|0|度量向 Microsoft.IoTSpaces 发出的 API 请求的延迟（以毫秒为单位）|无维度|
|FunctionExecutionLatency|否|FunctionExecutionLatency|6|0|度量 Microsoft.IoTSpaces 的用户定义函数执行的延迟（以毫秒为单位）|无维度|
|MessageEgressFailure|否|MessageEgressFailure|2|3|查找类似于 Microsoft.IoTSpaces 计数中“度量失败”计数事件的本地化字符串|无维度|
|MessageEgressLatency|否|MessageEgressLatency|6|0|为 Microsoft.IoTSpaces 度量从调度程序到其他终结点的延迟（以毫秒为单位）|无维度|
|MessageEgressSuccess|否|MessageEgressSuccess|2|3|查找类似于 Microsoft.IoTSpaces 计数中“度量已完成”计数事件的本地化字符串|无维度|
|ProcessingLatency|否|ProcessingLatency|6|0|为 Microsoft.IoTSpaces 度量从消息引入到调度事件的延迟（以毫秒为单位）|无维度|
...
|CbsVolumeBackupActive|是|卷备份是否已暂停|计数|平均值|卷的备份策略是否已暂停？ 如果是，则为 1；如果不是，则为 0。|无维度|
...
|ExpressRouteGatewayCpuUtilization|是|CPU 使用率（预览版）|计数|平均值|ExpressRoute 网关的 CPU 使用率|roleInstance|
...
|ExpressRouteGatewayPacketsPerSecond|否|每秒数据包数（预览版）|每秒计数|平均值|ExpressRoute 网关的数据包计数|roleInstance|
|P2SBandwidth|是|网关 P2S 带宽|每秒字节数|平均值|网关点到站点的平均带宽（字节/秒）|无维度|
