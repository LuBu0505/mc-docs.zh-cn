...
ms.reviewer: larryfr
ms.custom: deploy
ms.openlocfilehash: 35b0265f020b022892f28e24c220bf43ce385e42
ms.sourcegitcommit: 136164cd330eb9323fe21fd1856d5671b2f001de
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 03/05/2021
ms.locfileid: "102196919"
---
...
:::image type="content" source="./media/how-to-deploy-with-triton/triton-deploy.png" alt-text="仅使用 Triton 进行推理配置部署，不使用 Python 中间件":::
...
1. 直接通过 Triton 为模型提供服务。
1. 验证是否可以将请求发送到部署了 Triton 的模型。
1. （可选）为服务器端预处理和后期处理创建一个 Python 中间件层

## <a name="deploying-triton-without-python-pre--and-post-processing"></a>在没有 Python 预处理和后期处理的情况下部署 Triton
...
有关 `az ml model register` 的详细信息，请参阅[参考文档](/cli/ext/azure-cli-ml/ml/model)。
...
如果具有名为“aks-gpu”的已启用 GPU 的 Azure Kubernetes 服务群集（通过 Azure 机器学习创建），则可以使用以下命令部署模型。
...
[有关如何部署模型的更多详细信息，请参阅此文档](how-to-deploy-and-where.md)。

### <a name="call-into-your-deployed-model"></a>调用已部署的模型
...
执行基本运行状况检查之后，可以创建一个客户端，将数据发送到 Triton 以进行推理。 有关创建客户端的详细信息，请参阅 NVIDIA 文档中的[客户端示例](https://aka.ms/nvidia-client-examples)。 还可以参阅 [Triton GitHub 中的 Python 示例](https://aka.ms/nvidia-triton-docs)。

此时，如果你不想将 Python 预处理和后期处理添加到已部署的 Web 服务，那么你已完成操作。 如果要添加此预处理和后期处理逻辑，请继续阅读。
...
若要在部署中设置辅助角色数，请设置环境变量 `WORKER_COUNT`。 假设有一个名为 `env` 的 [Environment](https://docs.microsoft.com/python/api/azureml-core/azureml.core.environment.environment?preserve-view=true&view=azure-ml-py) 对象，可以执行以下操作：
