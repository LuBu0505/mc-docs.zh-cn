...
ms.openlocfilehash: f9ec40006984eee01ce65b760998adac3c69f3d9
ms.sourcegitcommit: 136164cd330eb9323fe21fd1856d5671b2f001de
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 03/05/2021
ms.locfileid: "102196527"
---
# <a name="trigger-machine-learning-pipelines"></a>触发机器学习管道

本文介绍如何以编程方式计划管道，使其在 Azure 上运行。 你可以创建基于已用时间或基于文件系统变更的计划。 基于时间的计划可用于处理例行任务，如监视数据偏移。 基于更改的计划可用于响应非常规或无法预测的更改，例如上传新数据或编辑旧数据。 了解如何创建计划后，你将了解如何检索和停用它们。 最后，你将了解如何使用其他 Azure 服务、Azure 逻辑应用和 Azure 数据工厂来运行管道。 Azure 逻辑应用允许使用更复杂的触发逻辑或行为。 使用 Azure 数据工厂管道可以将机器学习管道作为较大的数据业务流程管道的一部分进行调用。
...
由文件更改触发的管道可能比基于时间的计划更有效。 如果要在文件被更改前或者在将新文件添加到数据目录时执行某个操作，可以预处理该该文件。 可以监视对数据存储的任何更改，或监视数据存储中特定目录中的更改。 如果监视特定目录，该目录的子目录中的更改将不会触发运行。
...
在 Azure 数据工厂管道中，“机器学习执行管道”活动运行 Azure 机器学习管道。 可以在数据工厂的创作页中的“机器学习”类别下找到此活动：

:::image type="content" source="media/how-to-trigger-published-pipeline/azure-data-factory-pipeline-activity.png" alt-text="屏幕截图显示了 Azure 数据工厂创作环境中的 ML 管道活动":::
