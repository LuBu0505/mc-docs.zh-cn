...
ms.openlocfilehash: 861020fede145665e229f65b408a6d00fb0ef1d4
ms.sourcegitcommit: dc0d10e365c7598d25e7939b2c5bb7e09ae2835c
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 02/05/2021
ms.locfileid: "99579364"
---
...
现使用 [az vm create](/cli/vm#az-vm-create) 创建 VM。 使用 `--custom-data` 参数传递到 cloud-init 配置文件中。 如果未将 cloud-init.txt  配置文件保存在现有工作目录中，请提供该文件的完整路径。 以下示例创建一个名为 myVM  的 VM：
...
若要使 VM 能使用 Web 流量，请通过 [az vm open-port](/cli/vm#az-vm-open-port) 从 Internet 打开端口 80：
...
首先，使用 [az keyvault create](/cli/keyvault#az-keyvault-create) 创建 Key Vault，并在部署 VM 时启用该 Key Vault。 每个 Key Vault 均需具备唯一名称且全部小写。 将下例中的 `mykeyvault` 替换为自己唯一的 Key Vault 名称：
...
为供生产使用，应通过 [az keyvault certificate import](/cli/keyvault/certificate#az-keyvault-certificate-import) 导入由受信任的提供程序签名的有效证书。 在本教程中，以下示例显示了如何使用 [az keyvault certificate create](/cli/keyvault/certificate#az-keyvault-certificate-create) 生成使用默认证书策略的自签名证书：
...
若要在 VM 创建过程中使用该证书，请使用 [az keyvault secret list-versions](/cli/keyvault/secret#az-keyvault-secret-list-versions) 获取证书的 ID。 VM 需要特定格式的证书才能在启动时将其注入，因此请使用 [az vm secret format](/cli/vm) 转换该证书。 以下示例将这些命令的输出分配给变量，以便在后续步骤中使用：
...
现使用 [az vm create](/cli/vm#az-vm-create) 创建 VM。 使用 `--secrets` 参数注入 Key Vault 中的证书数据。 与上个示例一样，使用 `--custom-data` 参数传入 cloud-init 配置：
...
若要使 VM 能使用安全的 Web 流量，请通过 [az vm open-port](/cli/vm#az-vm-open-port) 从 Internet 打开端口 443：
