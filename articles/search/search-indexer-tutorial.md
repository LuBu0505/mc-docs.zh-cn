...
ms.openlocfilehash: 4ee25398328958af0ccc4c99435900de1a93b212
ms.sourcegitcommit: 6fdfb2421e0a0db6d1f1bf0e0b0e1702c23ae6ce
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 02/18/2021
ms.locfileid: "101087624"
---
...
1. 对于 `SearchServiceEndPoint`，如果服务概述页面上的完整 URL 为“https://my-demo-service.search.azure.cn”，则提供的值为该 URL。
...
索引架构定义字段集合，包含的属性用于指定允许的操作，例如字段是否可以进行全文搜索、筛选或排序，如以下针对 HotelName 的字段定义所示。 [SearchableField](https://docs.microsoft.com/dotnet/api/azure.search.documents.indexes.models.searchablefield) 按定义进行全文搜索。 其他属性是显式分配的。
...
主程序包含用于创建[索引器客户端](https://docs.microsoft.com/dotnet/api/azure.search.documents.indexes.models.searchindexer)、索引、数据源和索引器的逻辑。 此代码检查是否存在同一名称的资源，如果存在则会将其删除，所依据的假设是此程序可能多次运行。
...
索引器对象与平台无关，无论源是什么，配置、计划和调用都是相同的。 此示例索引器包含一个计划和一个用于清除索引器历史记录的重置选项，并调用一个方法来创建和立即运行索引器。 若要创建或更新索引器，请使用 [CreateOrUpdateIndexerAsync](https://docs.microsoft.com/dotnet/api/azure.search.documents.indexes.searchindexerclient.createorupdateindexerasync)。
...
索引器运行通常是有计划的，但在开发期间，你可能想要立即使用 [RunIndexerAsync](https://docs.microsoft.com/dotnet/api/azure.search.documents.indexes.searchindexerclient.runindexerasync) 运行索引器。
