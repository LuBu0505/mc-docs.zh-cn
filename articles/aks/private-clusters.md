...
3. “自定义专用 dns 区域名称”应采用此 azure 全球云格式：`privatelink.<region>.cx.prod.service.azk8s.cn`。 你将需要该专用 DNS 区域的资源 ID。  此外，你将需要一个用户分配的标识或服务主体，其中至少有自定义专用 dns 区域的 `private dns zone contributor` 角色。
