...
- 冷访问层中的数据具有略低的可用性，但仍具有类似于热数据的高持久性、检索延迟和吞吐量特征。 与热数据相比，冷数据的可用性略低且访问成本略高，这是可以接受的，因为毕竟其总体存储成本较低。 有关详细信息，请参阅[存储的 SLA](https://www.azure.cn/support/sla/storage/index.html)。
...
如果 blob 位于存档存储中，则 blob 数据处于脱机状态，不能读取或修改。 若要在存档中读取或下载 Blob，必须首先将其解除冻结到联机层。 不能创建存档存储中 Blob 的快照。 但是，Blob 元数据会保持联机和可用状态，因而可列出 Blob、其属性以及元数据。 不允许设置或修改存档中的 blob 元数据。 对于存档中的 Blob，仅以下操作有效：[获取 Blob 属性](https://docs.microsoft.com/rest/api/storageservices/get-blob-properties)、[获取 Blob 元数据](https://docs.microsoft.com/rest/api/storageservices/get-blob-metadata)、[列出 Blob](https://docs.microsoft.com/rest/api/storageservices/list-blobs)、[设置 Blob 层](https://docs.microsoft.com/rest/api/storageservices/set-blob-tier)、[复制 Blob](https://docs.microsoft.com/rest/api/storageservices/copy-blob) 和[删除 Blob](https://docs.microsoft.com/rest/api/storageservices/delete-blob)。
...
如果未发生访问层更改，请使用 Blob 属性“Last-Modified”来计算提前删除时间。 否则，请通过查看 Blob 属性（即“access-tier-change-time”）来使用最后一次将访问层修改为“冷”或“存档”的时间。 有关 Blob 属性的详细信息，请参阅[获取 Blob 属性](https://docs.microsoft.com/rest/api/storageservices/get-blob-properties)。
...
所选区域中提供了不同的访问层以及 Blob 级分层。 如需完整列表，请参阅 [Azure 产品（按区域）](https://azure.microsoft.com/global-infrastructure/services/?regions=china-non-regional,china-east,china-east-2,china-north,china-north-2&products=storage)。
