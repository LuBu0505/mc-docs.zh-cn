...
ms.topic: tutorial
ms.date: 03/10/2021
ms.openlocfilehash: 78d51d3f68f64ddb80c1509c7149ecad2d3f649f
ms.sourcegitcommit: 8b3a588ef0949efc5b0cfb5285c8191ce5b05651
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 03/22/2021
ms.locfileid: "104767533"
---
...
在开始学习本教程之前，请安装 [Azure CLI](/cli/install-azure-cli-windows)。

## <a name="create-a-storage-account-and-blob-container-with-azure-cli"></a>使用 Azure CLI 创建存储帐户和 Blob 容器

需要先创建常规用途存储帐户才能使用 Blob。 如果没有[资源组](/cli/group#az-group-create)，请在运行该命令之前创建一个资源组。 以下命令会创建存储容器并显示其元数据。 复制 ID。

```azurecli
az storage account create --name contosoblobstorage5 --resource-group contosoResourceGroup --location chinanorth --sku Standard_ZRS --encryption-services blob
```
...
```azurecli
az keyvault create --name contosoKeyVault10 --resource-group contosoResourceGroup5 --location chinanorth
```
...
此部分不能通过命令行完成。 按照此[指南](https://docs.microsoft.com/azure/databricks/scenarios/store-secrets-azure-key-vault#create-an-azure-databricks-workspace-and-add-a-secret-scope)操作。 需要访问 [Azure 门户](https://portal.azure.cn/#home)来执行以下操作：
...
此部分不能通过命令行完成。 按照此[指南](https://docs.microsoft.com/azure/databricks/scenarios/store-secrets-azure-key-vault#access-your-blob-container-from-azure-databricks)操作。 需要使用 Azure Databricks 工作区来执行以下操作：
